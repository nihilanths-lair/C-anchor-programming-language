...
int main(int argc, char **argv/*/ argv[] /*/) then
 ...
 if (!file)
 {
    ...
    if (!file) return -1;
    // Если отсутствует файл, записать в него базовые настройки для интерпретатора (VM)
    fputc(0x02, file);
    fclose(file);
    file = fopen("meta_info.cfg", "rb");
    if (!file) return -1;
 ...
 /*/ Режимы vCPU: /*/
 // 1 - Управление указателем на инструкцию ложится на плечи программиста (как в brainfuck)
 // 2 - Управление указателем на инструкцию происходит автоматически, но при желании можно переключиться на ручное управление (как в brainfuck)
