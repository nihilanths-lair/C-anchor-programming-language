...
void Preprocessing(const char * text)
{
    ...
    */
    ...
    // Для отладочной информации можно выбрать либо только замена макросов (их развёртка) с сохранением комментариев, либо удаление только комментариев с сохранением максоров
    // Для релизного выходного файла используется вариант с заменой макросов (их развёрткой) и удалением комментариев

    switch (3){ //char preprocessing_type = <?>; / режим
    case 1: {
