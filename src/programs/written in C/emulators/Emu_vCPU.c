...
void Preprocessing(const char * text)
{
    ...
    */
    ...
    // cdlr -E _.cdlr > путь и название итогового файла с расширением (с потерей комментариев)
    // cdlr -E -C _.cdlr > путь и название итогового файла с расширением (с сохранением комментариев)
    //[1] Препроцессорная обработка с заменой/подстановой текста (макросов), но с сохранением комментариев
    //[2] Препроцессорная обработка с заменой/подстановой текста (макросов) и с удалением комментариев
    //[3] Препроцессорная обработка с удалением комментариев, но с сохранением макросов
