...
void Executor_VM() // Spin / Executor (исполнитель) / Evaluator (древоходец)
{
    ...
    _42: // 2 байта | shl a8, i8 ; логический сдвиг влево (умножение на 2^i8)
    {
        PRINT_OPCODE();
        // Сдвигаем биты в аккумуляторе влево на количество позиций, указанное в _ip[1]
        a8 = a8 << _ip[1];
        _ip += 2;
        DISPATCH();
    }
    _43: // 2 байта | shr a8, i8 ; логический сдвиг вправо (деление на 2^i8)
    {
        PRINT_OPCODE();
        // В Си для знакового char (signed) сдвиг >> делает арифметический сдвиг (сохраняет знак).
        // Так как операция ЛОГИЧЕСКАЯ, нам нужно обнулять старшие биты.
        // Приводим к беззнаковому типу перед сдвигом, чтобы гарантированно вдвигать нули слева.
        a8 = (unsigned char)a8 >> _ip[1];
        _ip += 2;
        DISPATCH();
    }
    // Логические сдвиги битов /-//
