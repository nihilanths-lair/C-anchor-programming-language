...
void Executor_VM() // Spin / Executor (исполнитель) / Evaluator (древоходец)
{
    ...
    _42: // shl a8, i8 ; логический сдвиг влево (умножение на 2^i8)
    {
        PRINT_OPCODE();
        a8 = a8 << _ip[1]; // Сдвигаем биты в аккумуляторе влево на количество позиций, указанное в _ip[1]
        _ip += 2;
        DISPATCH();
    }
    _43: // shr a8, i8 ; логический сдвиг вправо (деление на 2^i8)
    {
        PRINT_OPCODE();
        // Для знакового char (signed) сдвиг >> делает арифметический сдвиг (сохраняет знак).
