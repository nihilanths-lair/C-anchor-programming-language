...
int main(int argc, char *argv[])
{
    ...
    int test_rules[] =
    {
        ...
        // Индекс 2: Если это НЕ тег (is_match == 0) — прыгаем на Индекс 8 (на шаг вперёд)
        OP__JUMP_IF_NOT_EQUAL, 8,

        // Индекс 4: ТЕСТ ГЕНЕРАЦИИ: если это тег, приказываем ядру ЗАПИСАТЬ команду OP__STEP_FORWARD в out_segment!
        OP__GENERATE_CODE, OP__STEP_FORWARD,

        // Индекс 6: Включаем саму инъекцию Си-кода
        OP__INJECTION_UNTIL_TAG,

        // Индекс 7: После инъекции прыгаем на Индекс 0, чтобы продолжить искать следующие теги
        OP__JUMP, 0,

        // Индекс 8: Сдвигаем курсор данных dp на +1 символ вперед (если это был обычный текст)
        OP__STEP_FORWARD,

        // Индекс 9: Безусловный прыжок обратно на Индекс 0 для проверки следующего символа
        OP__JUMP, 0
    };
