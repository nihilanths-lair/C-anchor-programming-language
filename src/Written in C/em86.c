...
{
    setlocale(0, "");
    char * test_program[] = // Тестовая программа для проверки флагов и условных переходов
    {
        "mov rcx, 50",    // Шаг 1: Кладем в RCX число 50
        "cmp rcx, 50",    // Шаг 2: Сравниваем RCX с 50. ZF должен стать 1!
        "je 8",           // Шаг 3: Если ZF=1, прыгаем на абсолютный адрес rip=8 (прямо на hlt)
        "mov rax, 999",   // Шаг 4: ЭТОТ ШАГ ДОЛЖЕН БЫТЬ ПРОПУЩЕН при успешном прыжке!
        "hlt"             // Шаг 5: Точка приземления прыжка (наш hlt сидит на rip=8)
    };
    int program_lines = 5;
    int v_rip = 0;
