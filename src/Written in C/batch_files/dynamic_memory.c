...
unsigned char * heap_mem_alloc(const unsigned char cell)
{
    ...
    case '0': // Слот свободен.
    {
        ...
        else // (*(__m+1) >= cell)
        {
            ...
            #endif
            // Разметим мета-данные: 3 байт-заголовка: флаг доступности №1, размер блока данных №1 и размер блока данных №2 (оставшуйся доступную память)
            *(__m+1+cell) = *(__m+1)-cell; // В след. участке обновим оставшийся размер данных, формула вычисления: текущий размер = старый размер - новый размер
            #ifdef MACRO__DEBUG_HEAP_MEM_ALLOC
            printf("\n %16X+[%d=(%X)] = %d ; Оставшееся место.", __m+1+cell, (__m+1+cell)-m, __m+((__m+1+cell)-m), *(__m+1+cell));
            #endif
