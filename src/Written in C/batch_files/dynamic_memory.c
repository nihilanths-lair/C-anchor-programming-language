...
unsigned char * heap_mem_alloc(const unsigned char cell)
{
    ...
    {
        printf("\n Поиск незанятого фрагмента памяти: [%d][%d]", *__m, *(__m + 1));
    ...
    {
        printf("\n Найден свободный фрагмент памяти : [%d][%d] <= %d", *__m, *(__m + 1), cell);
...
void heap_mem_free(unsigned char *m)
{
    ...
    printf("\n Пометим фрагмент памяти на доступность: <%p> | [%02Xh::%03dd]=%d", m, ((uintptr_t) m) % 256, ((uintptr_t) m) % 256, *m);
    printf("\n  <HEX> [%02X]=·%02X  [%02X]=·%02X", ((uintptr_t) m-2) % 256, *(m-2), ((uintptr_t) m-1) % 256, *(m-1));
    printf("\n <DEC> [%03d]=%03d [%03d]=%03d\n", ((uintptr_t) m-2) % 4096, *(m-2), ((uintptr_t) m-1) % 4096, *(m-1));
    *(m-2) = 0;
}
//
