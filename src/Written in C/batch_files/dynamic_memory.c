...
unsigned char * heap_mem_alloc(const unsigned char cell)
{
    ...
    while (*__m) // Пока участок занят, продолжаем искать свободный
    {
        ...
        printf("\n m(%02X+%02X|%03d+%03d) == m[%02X|%03d] = byte: %02X|%03d, number: %d ; Флаг занятости", // Флаг занятости
        ...
        printf("\n m(%02X+%02X|%03d+%03d) == m[%02X|%03d] = byte: %02X|%03d, number: %d ; Размер фрагмента данных", // Размер фрагмента данных
